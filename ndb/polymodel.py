...
class PolyModel(model.Model):
  ...
  def _from_pb(cls, pb, set_key=True, ent=None, key=None):
    ...
    """
    ent = super(PolyModel, cls)._from_pb(pb, set_key, ent, key)
    # Advanced Python acrobatics: Patch the __class__ attribute.
    # The alternative would be to dig through the pb looking for
    # properties named 'class_', which would require a linear search
    # and redundant decoding.
    ent.__class__ = cls._class_map[tuple(ent.class_)]
    return ent
